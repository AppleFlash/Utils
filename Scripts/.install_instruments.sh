...
check_var_empty UTILS_PATH
check_var_empty UTILS_BIN_PATH
...
read_current_git_state() {
    ...
    if [ ! -d "$cloned_dir_name/.git" ]; then
        ...
        CURRENT_GIT_COMMIT=$(git rev-parse HEAD) # current commit SHA
        cd - > /dev/null
    fi
}
...
clone_if_needed() {
    ...
    if [ ! -d "$cloned_dir_name/.git" ]; then
        echo "✨ Cloning $repo_url into $cloned_dir_name..."
...
safety_checkout_to_tag() {
    ...
    # safety check
    if [ ! -d ".git" ]; then
        echo "💥 Error: Directory \".git\" in \"$cloned_dir_name\" not found!"
        exit 2
    fi
    ...
    git checkout "tags/$target_tag" > /dev/null
...
main() {
    ...
    local cloned_dir_name="$UTILS_PATH/$util_name/"
    ...
    local moved_binary_path=$UTILS_BIN_PATH/$execulable_name
